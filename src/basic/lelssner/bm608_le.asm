...
; Initialisierung Treiber
	...
	ENDIF
		ld	hl, ttydrv
		ld	(0EFD1h), hl	; Adr. TTY-Treiber für READER
		ld	(0EFD9h), hl	; Adr. TTY-Treiber für PUNCH
		ld	hl, aBasicTape ; "BASIC-TAPE 1.00"
		ld	(0EFEBh), hl	; Zeichenkette für READER
		ld	(0EFEDh), hl	; Zeichenkette für PUNCH
	IF ERWTYP == "BASEA"
		xor     a
		ld      (68h), a
	ENDIF
...
rconb:		ld	d, 10		; RCONB: Eingabe einer Zeichenkette von CONST
		jr	conso

openr:		ld	d, 13		;OPENR: Eröffnen Kassette lesen
		jr	conso
...
rcons:		push	hl
		...
		add	hl, bc
		ld	b, 8		; max. Länge Dateiname
		push	de
		ld	de, 005Ch	; FCB
		ex	de, hl
; In FCB Leerzeichen zu	00-Bytes ändern
rcons1:		ld	a, (de)
		cp	20h ; ' '
		jr	nz, rcons2
		xor	a		; A:=0
...
rcons6:		ld	a, e
		...
		ld	hl, unk_E671
		add	hl, bc
		ld	b, (hl)		; B := logische	Gerätenummer (intern)
		ld	c, d
...
rcons8:		bit	6, e		; Endebit
		...
		jr	z, rcons9
		bit	5, e		; Rücksprung zum Steuerprogramm-Bit
		jr	z, rcons10
		res	5, e
...
rcons15:	bit	5, e		; Rücksprung zum Steuerprogramm-Bit
		jr	nz, rcons13
		ret
