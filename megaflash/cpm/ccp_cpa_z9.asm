...
;	Version CCP2 vom Z9001-CP/A !!!
;
; reass, f¸r AS-Assembler V.Pohlers 15.10.2014

; segment RAM
CLKBCD:		equ	001Dh		; BCD-Buffer f¸r CLK orig 50h, z9001 1Dh
COMREC:		equ	007Ch
...
COMBUF:
		...
		db "Zentralinstitut fuer Kernforschung KFM sbg",0Dh,0Ah
		db "CCP fuershadow BDOS    09.12.88",0Dh,0Ah
		db  1Ah
		Db 128-($-COMBUF) dup (0)

;	total buffer length is 128 characters
;
;;DISKA		EQU	0003H		;disk address for current disk		; 0003 Z9001, sonst 0004
...
;print the read error message
		ld	bc, aReadError	; "Read	Error\a$"
		jr	print
saverr:
		ld	bc, aNoSpace	; "No Space\a$"
		jr	print

renmsg:
		ld	bc, aFileExists	; "File	exists\a$"
		jr	print

nofile:
;print no file message
		ld	bc, aNoFile	; "No File$"
...
;check for a character ready at the console
		...
		xor	a
		ret
break_key1:	ld	bc, aBreak	; "*break*\a$"
		call	print
		xor	a
		inc	a
		ret
...
;found, but may be system file
		...
		pop	af
		inc	a
		and	1		; z9001 2th column? orig and 3
dircol:		equ $-1		
		push	af
		ld	a, ' '
		jr	nz, loc_CFCA
...
clk:
		ld	hl, CLKBCD
		ld	b, 3		; Z9001 nur 3 BCD-Werte (nur Uhr) orig 6
clk1:		push	bc
		push	hl
		call	getnumber
		pop	hl
...
readcom3:	ld	a, (hl)		;get character and translate
		...
		jr	readcom0
;
aReadError:	db "Read Error",7,'$'
aNoFile:	db "No File$"
ermsg:		db "ALL (Y/N)?$"
aNoSpace:	db "No Space",7,'$'
aFileExists:	db "File exists",7,'$'
aBreak:		db "*break*",7,'$'
;
