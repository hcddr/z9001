...
;;;; POHLERS-GRAFIK nach Kleinstrechnertipps 11


;--------GRAFP----------
GRAFZE		equ	24*8		; Anz. Zeilen
GRAFSP		equ	40*8		; Anz. Spalten
;^^^^^^^^GRAFP^^^^^^^^^^
...
; 8 KByte Speicher (320x192 Pixel), wird über Ports angesteuert
; Byte 0 wird links oben angezeigt: Pixel (191,0)-(191,7)
;OUT    GR_CTRL, 0    -> normaler Textmodus (wie nach Reset)
;OUT    GR_CTRL, 10h+zeile -> Vollgrafik an, Beschreiben Zeichen-Zeile 0..7
...
loc_9158:	ld	a, (unk_254)
		...
		and	a
		ret	z
		call	sub_954C	; Ausgabe A auf Plotter
		xor	a
		jp	sub_954C	; Ausgabe A auf Plotter
...
; Bitmaske ermitteln
sub_99E8:	ld	d, c		; de ist max 192, d.h. d ist immer 0	
		ld	a, c
...
; g_bitmask_y = Zeile mod 8
		ld	a, 7
		sub	e
		and	7
		ld	(g_bitmask_y), a

; g_memptr = letzte BWS-Zeile - 40*(Zeile/8) + (Spalte/8)
...
g_readbyte:	push	hl
		push	de
		ld	a, (g_bitmask_y)
		or	00001000b	; grafik ein
		out	(GR_CTRL),a
		ld	hl, (g_memptr)	; aktuelle Adresse im Grafikspeicher
		ld	a, (hl)
; Beschränken HL auf Bereich EC00-EFBF
		ld	de, 0EC00h
		call	cpreg		; HL-DE?
		jr	c, g_readbyte0	; wenn HL<DE
		ld	de, 0EC00h+40*24
		call	cpreg		; HL-DE?
		jr	nc, g_readbyte0	; wenn HL>=DE
		ld	a,(hl)		
		jr	g_readbyte2
g_readbyte0	ld	a, 0
g_readbyte2	pop	de
...
; Beschränken HL auf Bereich EC00-EFBF		
		ld	de, 0EC00h
		call	cpreg		; HL-DE?
		jr	c, g_writebyte2	; wenn HL<DE
		ld	de, 0EC00h+40*24
		call	cpreg		; HL-DE?
		jr	nc, g_writebyte2; wenn HL>=DE
		pop	af
		ld	(hl), a
; Farbattribut
		ld	a,h
		sub	4
		ld	h,a
		ld	a, (atrib)	; aktuelles Farbattribut
		ld	(hl), a
...
;
		...
		ld	hl, (g_memptr)	; aktuelle Adresse im Grafikspeicher
		ld	de, GRAFSP/8
		add	hl, de		; vorherige Zeile
		ld	(g_memptr), hl	; aktuelle Adresse im Grafikspeicher
		pop	de
		pop	hl
;
		ld	a, 0
g_prvrow2:	ld	(g_bitmask_y),a 
		and	a
		ret
...
;--------GRAFP----------
		ld	b,8
		ld	a, 00001000b	; Grafikmode einschalten und Bank einblenden
g_gcls1:	out	(GR_CTRL),a
		push	af
		push	bc
		ld	hl, 0ec00h
		ld	de, 0ec01h
