...
; V.Pohlers 26.05.2009 Grafiktreiber für Bildschirm 80x48 Pixel (Viertelgrafik)
; 09.06.2009 pset und circle funktionieren
; 11.06.2009 g_point funktioniert
; 16.06.2009 g_line funktioniert (endlich!)
; 07.07.2020 cls hat auch MAPPI überschrieben (EFC0), nun 1 Byte weniger in g_gcls
...
;--------GRAF11----------
GRAFZE		equ	48		; Anz. Zeilen
GRAFSP		equ	80		; Anz. Spalten
;^^^^^^^^GRAF11^^^^^^^^^^
...
;------------------------------------------------------------------------------
; 1/4-Grafik, nach A. Zierott, aus BAC87
; 80x48 Pixel
;------------------------------------------------------------------------------
...
sub_9A12:
zpset0:		push	hl		; wegsichern
		push	bc
		ld	hl,(g_memptr)
		push	hl		; g_memptr
		ld	a, (hl)		; aktuelles Grafikzeichen
		ld	hl, zpchtab	; Tabelle Pixel-Grafik-Zeichen
		ld	bc, 10h
		push	hl		; zpchtab
		cpir
...
zpset3:		xor	0Fh
		...
		pop	hl		; zpchtab
		add	hl, bc
		ld	a, (hl)		; neues Grafikzeichen
		pop	hl		; Position im BWS, g_memptr
		ld	(hl), a		; Grafikzeichen setzen
		pop	bc
		pop	hl
		ret
; End of function pset
...
;     FF  BD  BC  B6  BA  B5  B9  B1  BB  B8  B4  B0  B7  B2  B3  20
;
; 48  ##  ##  ##  ##  .#  .#  .#  .#  #.  #.  #.  #.  ..  ..  ..  ..
; 12  ##  .#  #.  ..  ##  .#  #.  ..  ##  .#  #.  ..  ##  .#  #.  ..
...
; spalte
; UP zu sub_9DC4 (line)
g_nxtcol:	ld	a,(g_line_x)
		inc	a
		ld	(g_line_x),a
		ret

; RET-UP in g_line
g_prvcol:	ld	a,(g_line_x)
		dec	a
		ld	(g_line_x),a
		ret
...
; Zeile
;??? ungenutzt	
g_prvrow:	ld	a,(g_line_y)
		dec	a
		ld	(g_line_y),a
		ret

; UP zu g_line
g_nxtrow:	ld	a,(g_line_y)
		inc	a
		ld	(g_line_y),a
		ret
...
loc_9E21:	call	sub_9A12a	; Byte in Grafikspeicher schreiben (Pixel setzen)
		dec	bc
		push	hl
		ld	hl, (unk_7FFCh)
		add	hl, de
...
g_point:	ex	de, hl
		...
		ld	bc, (g_memxpos)
		ld	de, (g_memypos)
		call	loc_99F5	; Umrechnung D-xpos, E-ypos zu g_memptr
		call	g_readbyte
;--------GRAF11----------
		ld	hl, zpchtab	; Tabelle Pixel-Grafik-Zeichen
		ld	bc, 10h
		...
		and	c
		ld	b, 0
		jr	z, g_point2 	; Wenn Bit nicht gesetzt (B = 0)
