...
;**************************************************************
;*   für as umgesetzt vp 11.07.03                             *
;*   eine Änderung gegenüber der Original-Quelle hcbas.mac    *
;*   bei WRINPB:                                              *
;* 	LD HL,-EOINPB+1   statt    LD HL,-EOINPB              *
;*   für bitgenaue Gleichheit zum PROM U2364 D45 BM600        *
;**************************************************************
...
;????.xcref
;	TITLE	HC-BASIC ROM-Version 7. 1. 1987
;	ASEG

;************************************************
	IF ROMTYP == "RAM"
	ROMBEG:	EQU	0300H		;INTERPRETERANFANGSADR.
	RAMBEG:	EQU	2B00H		;RAMANFANGSADR.
	WSP:	EQU	RAMBEG
...
;------------------------------------------------
	ELSEIF	ROMTYP	== "RAMH"
;;	ROMBEG:	EQU	09000H		;INTERPRETERANFANGSADR.
	ROMBEG:	EQU	08800H		;INTERPRETERANFANGSADR.
	RAMBEG:	EQU	300H		;RAMANFANGSADR.
	WSP:	EQU	RAMBEG
	PRAM:	EQU	WSP+257		;PROGRAMMSPEICHERANFADR.
	ZFT:	EQU	0		;WSP<PRAM
	BIN0IO:	EQU	1		;BINAERE NULLEN SIND
;ALS I/O-DATEN MOEGLICH
	EXTGO1:	EQU	ROMBEG + 02000H
	EXTGO2:	EQU	EXTGO1 + 03h
	EXTGO3:	EQU	EXTGO1 + 06h
	EXTTAB:	EQU	EXTGO1 + 09h
	VERT:	EQU	EXTGO1 + 0BH
;------------------------------------------------
	ENDIF
...
;
	...
	shared	DATFLG
	shared	EXTGO1
EXTBEG:	EQU	ROMBEG+2000H		;INTERPRETERANFANGSADR.
	shared	EXTBEG
	shared	CHARO
;für GRAF.COM
	shared	IOTEST	
...
DSTMNT	EQU	DATYPE+1	;03AF	DATA-STATEMENT-FLAG
...
RAMSH1:	equ	ramsch+3
	...
	LD	HL,RAMBEG-1
	JR	NC,RAMSH1
RAMSCH:	LD	DE,(36h)		;EOR

	ELSEIF BASTYP == "BASIC_84"
...
; vorhanden sind, dan beginnen diese mit LD A,B = 78H
;
	LD	HL,(EXTGO1+1)	;Proc. ERW1
	LD	A,(HL)		
	CP	A, 78H		;ist hier Code "ld a, b" ?
	JR	NZ,RAMSH0
...
;FOR DIV;DVAR1
	...
	DEFB	27		;PRLISI	;(ANZAHL DER DARSTELL-
;BAREN FORMATE PRO ZEILE -1)*
;FORMATLAENGE (13) + 1
	DEFB	0		;OUTFLG	;AUSGABEUNTERDRUECKUNG
;WENN <> 0
	DEFW	10		;LINFLG	;ZEILENANZAHL FUER LIST
	DEFW	10		;LNFLGC	;COPY VON LINFLG
	DEFW	0		;CHSUML	;PRUEFSUMME FUER
;CSAVE* UND CLOAD*
	JP	EING		;INPLOC	;EINGABEROUTINE FUER
;DATEN
...
ERRTAB:	DB	"NFSNRGODFCOVOMULBSDD"
	DB	"/0IDTMOSLSSTCNUFMO"
	DB	"IO"
ERR:	DB	" ERROR"
	DEFW	ASCBEL
IN:	DB	" IN "
...
ERROO1:	CALL	TEXTO		; anzeigen
	...
	CALL	CPREG
	JP	Z,PRIST1	;SPRUNG, WENN FEHLER BEI DER EINGABE
...
EDIT:	XOR	A
	...
	SCF
;ZEILENEINGABE
LININP:	CALL	C,CLCHNL	; alle benutzten Kanaele schliessen
	CALL	INPINI
	DEC	HL		; -1
...
;Z-FLAG=1, WENN ZEILENNUMMER SCHON VORHANDEN
...
SCANNE:	LD	HL,INPBUF
	...
	RET
;
;ROUTINE ZUR EINGABE EINER ZEILE UND ABLAGE
;IM EINGABEPUFFER
...
;ZEILENLAENGE UND MERKE POSITION DES WAGENS
CHARO:	PUSH	AF
	PUSH	BC
	LD	C,A
	CP	A, ' '
...
LIST:	CALL	IOTEST		; LIST-Schutz gesetzt?
	...
	POP	HL
LIST0:	CALL	DCHEX		; in Hex-Zahl umwandeln
	RET	NZ
	POP	BC
...
LIST9:	PUSH	HL			;ZEILENANZAHLKONTROLLE
	LD	HL,(LINFLG)
	LD	A,L
	OR	H
	DEC	HL
...
LIST11:	SUB	7FH
	CP	A, LAST-7FH+1
	JR	C,LIST10		;token aus Erweiterungsliste?
	SUB	LAST-7FH
	LD	DE,(EXTTAB)		;ja
	JR	LIST13
LIST10:	LD	DE,MNEMTB		;nein
LIST13:	LD	B,A
LIST12:	LD	A,(DE)
	INC	DE
	OR	A		; Bit 7 gesetzt?
...
DCLI2:	SUB	ENDTOK
	...
	SUB	LODTOK-ENDTOK
	JR	C,CPSTX2
	CP	A, LAST-LODTOK+1		;Kommando aus Grundliste?
	JR	C,DLI20		;ja
	LD	B,A
	LD	A,(EXTFLG)
	AND	A, A		;Erweiterung vorhanden?
...
READ12:	JP	READ4
;
;VORRANGSTEUERUNG DR OPERATOREN UND SCHLEIFENABSCLUSS
...
SNLY19:	LD	B,0
	...
	CALL	TCHAR
	LD	A,C
	CP	A, 1+MIDTOK-SGNTOK+MIDTOK-SGNTOK
	JR	C,SNLY29
	LD	A,(EXTFLG)
	AND	A, A
	JP	NZ,EXTGO3		;TOKEN NICHT IN GRUNDLISTE
SNLY29:	CP	A, PITOK-SGNTOK+PITOK-SGNTOK
	JR	Z,SNLY2A
	CP	A, 1+CHRTOK-SGNTOK+CHRTOK-SGNTOK
	JR	C,SNLY20
	CALL	SNALY6
	CALL	CPCOMM
	CALL	SNALY4
...
CLOA_1:	LD	(HL),0			;ALLE LAENGEN = 0
	...
	SBC	HL,DE
	PUSH	HL
	CALL	INIT8I		; Datei oeffnen, 1. Byte lesen
	LD	C,A
	CALL	CI
	LD	B,A
	POP	HL
...
CLOA_5:	DEC	(HL)
	...
	JR	NZ,CLOA_4
	POP	HL
CLOA_8:	CALL	CLCHNL		; benutzte Kanaele schliessen
	RET
CLOA_7:	POP	DE
	POP	HL
CLOA_6:	LD	(HL),0
...
CLOAD:	CP	A, MULTOK	; CLOAD* ?
	JR	Z,CLOADD	; dann Variablenfeld
;*.SSS Programm einlesen
	CALL	CASS01		; Name holen
	LD	DE,(SVARPT)
	DEC	DE
	DEC	DE		; DE=Ladeadresse
	LD	HL,0
	ADD	HL,SP
	LD	BC,-47
	SBC	HL,DE
	ADD	HL,BC
	LD	B,H
	LD	C,L		; BC=max. Anzahl
	CALL	INIT8I		; Datei oeffnen, erstes Byte lesen
	LD	L,A
	...
	SBC	HL,BC
	POP	HL
	JP	NC,OMER		; passt nicht in freien Speicher!
CLOAD1:
	CALL	CI
...
; ab INTPRB:
; 2 Byte IOERR
; 3 Byte Header-Kennung D4D4D4, D3D3D3 o. + Schreibschutz
; 8 Byte Dateiname (mit Leerzeichen aufgefüllt)
CASS0:	LD	A,D4TYPE	; *.TTT = Variablenfeld
	INC	HL
	DEFB	1		; folgenden Befehl überspringen
CASS01:	LD	A,D3TYPE	; *.SSS = BASIC-Programm
CASS1:	PUSH	AF		; Dateityp
	...
	JR	Z,CASS5
	POP	AF
	ADD	A,4		; Schreibschutzkennung addieren
	PUSH	AF
CASS5:	CALL	TCHAR1		; Pointer auf nächstes signifikantes Zeichen
	CALL	SNALY		; String-Parameter übernehmen
	POP	AF
	EX	(SP),HL
	PUSH	HL
	PUSH	AF
	CALL	ASC1		; String holen
	LD	HL,INTPRB+13	; PRINTPUFFER ENDE
				; Print-Buffer mit Leerzeichen füllen
	LD	B,13
CASS2:	DEC	HL
	LD	(HL),' '	; Name loeschen
	DJNZ	CASS2
	PUSH	DE
	LD	DE,IOERR	; IOERR in Print-Buffer ablegen
	LD	(HL),E		; HL=INTPRB
	INC	HL
	LD	(HL),D
	POP	DE
	INC	HL
	POP	AF		; 3x Header-Kennung schreiben
	LD	(HL),A
	INC	HL
	LD	(HL),A
	INC	HL
...
CASS3:	LD	A,C		; Anzahl Zeichen
	CP	A, 9		; kleiner als 9
	JR	C,CASS4		; dann kopieren
	DEC	C
	INC	HL		; ansonsten nur die letzten 8 Zeichen
	JR	CASS3
CASS4:	LDIR			; Dateinamen kopieren
	RET

;Prüfsumme bilden
;ab (HL) DE Bytes aufsummieren, Ergebnis nach BC
CHSUM:	PUSH	HL
	XOR	A
	LD	B,A
CHSUM1:	ADD	A,(HL)
...
INIT8I:	LD	A,(IOCHNL)
	BIT	IM_,A		; Datei geoeffnet?
	SET	IM_,A		; Kanal aktiv
	LD	(IOCHNL),A
	LD	A,ICH_+BIT8	; 8-Bit-Eingabe
	JR	NZ,SETOI1
	ADD	A,INIT		; Datei oeffnen
SETOI1:	LD	(ININD),A
;------------------------
	LD	HL,INTPRB	; Druckpuffer=Dateiname
	JP	CI
...
; Speichern
CSAVE:	CP	A, MULTOK	; '*', also Daten speichern?
	JR	Z,CSAVED	; ja, Variablenfeld
	CALL	IOTEST		; LIST/EDIT/BYE-Schutz?
	CALL	CASS01		; Fileheader erstellen
	LD	HL,(SVARPT)	; Programmende + 1
	LD	DE,-PRAM	; Programmanfang
	ADD	HL,DE
	LD	B,H
	LD	C,L		; BC = HL = Programmlänge
	LD	DE,PRAM		; DE = Programmbeginn
CSAVE0:	CALL	CSAVE2		; Ausgabe HL Bytes ab (DE)
CSAVE4:	POP	HL
...
; Datenfeld speichern mit CSAVE*
CSAVED:	CALL	CASS00		; Fileheader erstellen
	JP	PO,CSAVE_
	CALL	CHSUM		;Prüfsumme ab (HL) DE Bytes aufsummieren, Ergebnis nach BC
	EX	DE,HL
	SBC	HL,DE
	JR	CSAVE0
...
LAENG1:	LD	A,(HL)
	...
	INC	HL
	INC	HL
	CALL	CHSUM2		;in BC summieren, HL erhöhen
	JR	NZ,LAENG1	;wenn HL=DE
	POP	HL
	RET
...
; Ausgabe HL Bytes ab (DE)
CSAVE2:	INC	HL
	PUSH	HL
	LD	A,C
	CALL	INIT8O		; 8 Bit-Ausgabe
	LD	A,B
	POP	HL
CSAVE1:
	CALL	CO
...
ICHQ	EQU	04		; Eingabe Kanal 2 = Anwenderkanal 1
OCHQ	EQU	05		; Ausgabe Kanal 2 = Anwenderkanal 1
ICHQQ	EQU	06		; Eingabe Kanal 3 = Anwenderkanal 2
OCHQQ	EQU	07		; Ausgabe Kanal 3 = Anwenderkanal 2
...
;TEST AUF UNTERBRECHUNGSANFORDERUNG VOM CICH
CSTS:	PUSH	DE
	LD	E,CICH+IMRET
...
WRINPB:	INC	BC
	LD	(BC),A
	PUSH	HL
	IF (BASTYP == "BASIC_84") && (ROMTYP == "ROM")
	LD	HL,-EOINPB
	ELSE
	LD	HL,-EOINPB+1	;vp: so in original BM600
	ENDIF
	ADD	HL,BC
	POP	HL
	RET	NC
...
ZECR:	CP	A, ASCCR
	RET	NZ
ZECR0:	LD	A,CURRGT		;Ende nach CR
	CALL	CO
	INC	HL
	LD	A,(HL)
	OR	A
...
ZECR1:	JP	CRWDY6
;--------------------------------
ZSTOP:	CP	A, STOPSW			;Ende nach STOP
	JR	Z,ZSTOP1
	AND	A, A
	RET
ZSTOP1:	CALL	ZECR0
...
ZEEDI:	CP	A, CURLFT
	...
	RET
;--------------------------------
ZEDI42:	LD	(HL),A		;Zeichen eintragen
ZEDI4:	CALL	CO		;CURRGT
	INC	HL
	LD	A,(HL)
	OR	A
	RET	NZ
