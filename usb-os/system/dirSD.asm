...
dirkdo0a:	call	sdinit		; Synchronisation
...
rda:		ld	hl,memry
		; entry type oder 00 (Ende)
		call	getbyte		; Zeichen holen
		cp	0		;ende?
		jp	z,exit		; Prompt erkannt -> Anzeige

		ld	(hl),a		;memry = D/F entry type dir/file
		inc	hl
		; dateiname "8.3"
		ld	b,12
rd1:		call	getbyte		; Zeichen holen
...
rd2:		ld	(hl),0		; Ende - 0 Byte f. Stringausgabe
		call	setcolor
;Pseudo-DIR '.' oder '..'?		
		ld	hl,memry+1
		ld	a,(hl)
...
; Ablegen in Speicher
; nach (HLSV) kopieren
rdhl:		ld	hl,memry	; Typ+8.3+0 kopieren
		ld	de,(HLSV)
rdhl3:		ld	a,(hl)
		ldi
		cp	0
		jr	nz, rdhl3
		ld	(HLSV),de
		ld	(de),a		; abschließendes 0-Byte schreiben
		jp	rda
;------------------------------------------------------------------------------
;
;nächstes Zeichen aud Puffer holen, Trenner und Ende beachten
nxtch1:		ld	a,(hl)
		inc	hl		; sonst
		ret
...
;------------------------------------------------------------------------------
; Ende bei Abbruch
exit0:		call	getbyte		; restl. Zeichen holen
		or	a		; 0?
		jr	nz,exit0	;nein - weiter 
; Ende
exit:		call	c_gruen
		call	ocrlf
		xor	a
...
setcolor:	;HL steht hinter Dateiname, also 4 Zeichen zurück
		dec	hl
		ld	a,(hl)
		dec	hl
		cp	'M'
...
cdkdo:
	...
	push	de
	ld	b, 0		;Länge des Pfadnamens ermitteln und in B speichern
cdkdo0:	ld	a, (de)
	cp	' '		;Ende bei Leerzeichen
	jr	z, cdkdo1
	cp	0		;Ende bei NULL
	jr	z, cdkdo1
	inc	b		;Länge und String Pointer erhöhen
	inc	de
